...
def run_task(snapshot_config, variant_data, *_):
    ...
    """
    with LocalTFRunner(snapshot_config=snapshot_config, max_cpus=4) as runner:

        env = PixelObservation(gym.make('LunarLanderContinuous-v2'))
        env = Grayscale(env)
...
def _args(buffer_size, seed):
    ...
    """
    return buffer_size, seed

buff_size = _args.main(standalone_mode=False)[0]
seed = _args.main(standalone_mode=False)[1]

replay_buffer_size = buff_size
run_experiment(
    run_task,
    snapshot_mode='last',
    seed=seed,
    plot=True,
    exp_name='dppg_pendulum_pixels_seed_{}'.format(seed),
